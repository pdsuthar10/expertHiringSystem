...
class WaterRepairCart extends Component {
    constructor(props){
        super(props);
  
        this.state = {
          services: ['Repair', 'Installation/Fitting','Servicing/Filter','Cleaning'],
          products: []
        };
    }
    ...
    getList = (name) =>{
        switch (name) {
            ...
            case 'Servicing/Filter':
                return global.wServiceList;
            case 'Cleaning':
                return global.wCleaningList;
            default:
                break;
        }
    }
    ...
    render() {
        ...
        for(var product in this.state.products){
            if(this.state.products[product].category === 'Cleaning')
                cleaningCategory.push(this.state.products[product])
        }
        ...
        global.wInstallList = (serviceCategory.length) ? (
        ...
        global.wCleaningList = (cleaningCategory.length) ? (
            cleaningCategory.map(product => {
                return(
                   <ProductCard product={product}/>
                ) 
            })
...
export default connect(mapStateToProps,{ toggleCartModal, addProductFromDB })(WaterRepairCart);